...
export default function decorate(block) {
  ...
  if (!window.location.href.includes('/our-funds/')) {
  ...
  // );
  dataMapMoObj.getscheme(planslabel).then((resp) => {
    const respText = [resp];
    dataMapMoObj.schemeCodeResp = respText;
    try {
      dataMapMoObj.CLASS_PREFIXES = [
        'compound-item',
        'compound-sub-item',
      ...
      stky.querySelector('.sticky-inner-item1').textContent = respText[0].schDetail.schemeName;
