...
export function calculateSipDelaySummary({
  ...
  const calculateFVA = (P, i, n) => {
    ...
    const fvaFactor = (Math.pow(1 + i, n) - 1) / i;
...
function recalculateSipDelay({
  ...
  const monthlyInvestment =
    mi != null ? Number(mi) : Number(miInput?.value || 0);
  const annualReturnRate =
    ror != null ? Number(ror) : Number(rorInput?.value || 0);
  const years = ip != null ? Number(ip) : Number(ipInput?.value || 0);
  const delayYears =
    delay != null ? Number(delay) : Number(delayInput?.value || 0);
...
export default function decorate(block) {
  ...
  const miBlock = createInputBlock({
    ...
    onInput: (e) =>
      recalculateSipDelay({ mi: e.target.value, container: block }),
  ...
  const ipBlock = createInputBlock({
    ...
    onInput: (e) =>
      recalculateSipDelay({ ip: e.target.value, container: block }),
  ...
  const rorBlock = createInputBlock({
    ...
    updateWidthonChange: true,
    onInput: (e) =>
      recalculateSipDelay({ ror: e.target.value, container: block }),
  ...
  const delayBlock = createInputBlock({
    ...
    updateWidthonChange: true,
    onInput: (e) =>
      recalculateSipDelay({ delay: e.target.value, container: block }),
