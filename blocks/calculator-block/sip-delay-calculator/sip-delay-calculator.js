 ...
 * @param {Object} params
 * @param {number} params.monthlyInvestment - Amount invested per month
...
export function calculateSipDelaySummary({
  ...
  // Total if started today
  for (let m = 1; m <= months; m += 1) {
    todayTotal += monthlyInvestment * (1 + monthlyRate) ** (months - m + 1);
  }

  // Total if delayed
  for (let m = 1; m <= months - delayMonths; m += 1) {
    delayTotal
      += monthlyInvestment * (1 + monthlyRate) ** (months - m - delayMonths + 1);
  }
