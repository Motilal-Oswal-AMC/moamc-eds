...
export function createInputBlock({
  ...
  const inputAttrs = {
    ...
    onbeforeinput: (e) => {
      // Prevent non-numeric input for numeric field types
      if (['currency', 'percent', 'year'].includes(fieldType)
          || ['number', 'stepper'].includes(variant)) {
        if (e.data && !/^\d$/.test(e.data)) {
          e.preventDefault();
        }
      }
      // Also prevent decimal points for integer fields
    ...
    onkeydown: (e) => {
      ...
      // For numeric field types, only allow numbers and allowed keys
      if (['currency', 'percent', 'year'].includes(fieldType)
          || ['number', 'stepper'].includes(variant)) {
  ...
  const getRealInput = () => (fieldType === 'currency'
