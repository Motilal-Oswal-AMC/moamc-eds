...
export function calculateSTPSummary({
  ...
  const msaAmount =
    num(transferAmount) || container.querySelector("#msa-amount").value * 1;
  const erftsStart =
    num(sourceAnnualRate) || container.querySelector("#erfts-start").value * 1;
  const erfssStart =
    num(destinationAnnualRate) ||
    container.querySelector("#erfss-start").value * 1;
  const tosiStart =
    num(years) || container.querySelector("#tosi-start").value * 1;
  ...
  for (let i = 1; i <= months; i += 1) {
    ...
    destinationBalance =
      (destinationBalance + transfer) * (1 + destinationMonthlyRate);
  }
