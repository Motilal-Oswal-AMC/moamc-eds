...
export default function decorate(block) {
  ...
  const CALC_RECOMMENDATIONS_ITEMS =
    CALC_RECOMMENDATIONS_CONTAINER.querySelector("ul");

  const RECOMMENDED_GOAL_BANNER =
    CALC_RECOMMENDATIONS_CONTAINER.querySelectorAll("ul")[1];
  ...
  const RECOMMENDED_GOAL_BANNER_ITEMS =
    CALC_RECOMMENDATIONS_CONTAINER.querySelectorAll(
  ...
  const CALC_INSIGHT =
    CALC_INSIGHT_N_DISCLAIMER.querySelector("ul:first-child");
  ...
  if (RECOMMENDED_FUNDS) {
    ...
    const filteredPlans = tempDfilt.filter(
      (card) =>
        Array.isArray(card.planList) &&
        card.planList.some((plan) =>
          plan.planName?.toLowerCase().includes("direct")
        )
    );
    filteredPlans.slice(0, getCardsPerRow({ padding: 0 })).forEach((card) => {
