...
export default function decorate(block) {
  ...
  if (
    ...
    // CALC_AUTHOR_MAIN.style.height = `100%`; // temporarily fix height
  ...
  if (RECOMMENDED_FUNDS) {
    ...
    const filteredPlans = tempDfilt.filter(
      (card) => Array.isArray(card.planList)
        && card.planList.some((plan) => plan.planName?.toLowerCase().includes('direct')),
    );
  ...
  document.addEventListener('click', (event) => {
    if (
      event.target.classList.contains('selectedtext')
