  ...
  submitButton.addEventListener('click', async (e) => {
    ...
    // console.log('Form is valid. Submitting...');
      try {
        ...
        if (result) {
        // alert
          console.log(div('Your details have been submitted successfully!'));
          // Reset form
          fields.forEach((f) => {
            f.value = '';
        ...
        // alert
          console.log(div(`Something went wrong: ${result.message || 'Unknown error'}`));
        }
      } catch (error) {
      // console.error('API Error:', error);
        console.log(div('Failed to submit form. Please try again later.'));
      }
    } else {
      toggleSubmitButton();
    }
